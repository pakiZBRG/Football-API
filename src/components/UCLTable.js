...
export default function UCLTable({groups}) {
    return (
        <React.Fragment>
            {groups.map((group, i) => (
                <div key={i}>
                    <h2 className='center' style={{marginTop: '1rem'}}>{group[0].group}</h2>
                    <table className='table-league'>
                        ...
                        <tbody>
                            ...
                            const promotion = type => description && !description.search(type) ? 'promotion' : null;
                            const relegation = () => description && !description.search('Relegation') ? 'relegation' : null;
